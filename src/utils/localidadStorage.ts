...
export const saveLocalidad = (localidad: Omit<LocalidadData, 'createdAt' | 'updatedAt'>): LocalidadData => {
  try {
    ...
    const existing = localidad.id ? localidades.find(l => l.id === localidad.id) : null;
...
export const updateLocalidadStatus = (
  id: string, 
  status: LocalidadData['status'],
  usuario?: string
): boolean => {
  try {
    const localidades = getAllLocalidades();
    ...
    const localidadAnterior = localidades[localidadIndex];
    const statusAnterior = localidadAnterior.status;
...
export interface HistorialCambio {
  ...
  valorAnterior: string | number | string[];
  valorNuevo: string | number | string[];
  accion: 'created' | 'updated' | 'published' | 'archived' | 'deleted';
}

const HISTORIAL_KEY = 'ecorutas_historial_cambios';
...
export const registrarCambio = (
  ...
  valorNuevo: string | number | string[],
  usuario: string,
  accion: HistorialCambio['accion'] = 'updated'
): HistorialCambio => {
  try {
    const historial: HistorialCambio[] = JSON.parse(localStorage.getItem(HISTORIAL_KEY) || '[]');
    ...
    const nuevoCambio: HistorialCambio = {
      ...
      valorNuevo: typeof valorNuevo === 'object' ? JSON.stringify(valorNuevo) : String(valorNuevo),
      accion,
    };

    historial.unshift(nuevoCambio); // Agregar al inicio
...
// Función para obtener historial de una localidad
export const obtenerHistorialLocalidad = (localidadId: string): HistorialCambio[] => {
  try {
    const historial: HistorialCambio[] = JSON.parse(localStorage.getItem(HISTORIAL_KEY) || '[]');
    return historial.filter(h => h.localidadId === localidadId);
  } catch (error) {
...
// Función para comparar y registrar cambios entre dos objetos
export const registrarCambiosDesdeComparacion = (
  localidadId: string,
  datosAnteriores: Partial<LocalidadData>,
  datosNuevos: Partial<LocalidadData>,
  usuario: string
): void => {
  const camposAComparar: (keyof LocalidadData)[] = [
    ...
    'sitioWeb', 'redesSociales', 'precioPorPersona', 'imagenes', 'status'
  ];
  ...
  camposAComparar.forEach(campo => {
    ...
    // Normalizar valores para comparación (tratar undefined/null como string vacío)
    ...
    if (Array.isArray(valorAnteriorNormalizado) && Array.isArray(valorNuevoNormalizado)) {
      ...
      registrarCambio(localidadId, String(campo), valorAnteriorStr, valorNuevoStr, usuario);
    }
  });
};
